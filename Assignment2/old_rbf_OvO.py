...
def my_predict(mesh,x_train,y_train,g_opt):
	row = []
	for m in M:
		y_train_norm = []
		...
		clf = svm.SVC(kernel='rbf',gamma=g_opt)
...
accuracies = []
for g in G: # Grid Search
	accuracy = 0
	confusion_matrix = np.zeros((len(M),len(M)))
	for i in range(k): # k cross validation
		x_train = []
		y_train = []
...
# fig.savefig('Plots/svmRbfOvO-decision-'+args.data.strip().split('_')[-1][:-3]+'.png')


for i in range(len(sv)):
	plt.plot(sv[i][0],sv[i][1],'g+')
fig.savefig('Plots/svmRbfOVO-DB-SV-'+args.data.strip().split('_')[-1][:-3]+'.png')