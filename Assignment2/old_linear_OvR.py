...
def my_predict(mesh,x_train,y_train,c_param):
	row = []
	for m in M:
		y_train_norm = []
		...
		clf = svm.SVC(kernel = 'linear',C=c_param)
...
# fig.savefig('Plots/svmLinearOvR-decision-'+args.data.strip().split('_')[-1][:-3]+'.png')

for i in range(len(sv)):
	plt.plot(sv[i][0],sv[i][1],'g+')
fig.savefig('Plots/svmLinearOVR-DB-SV-'+args.data.strip().split('_')[-1][:-3]+'.png')